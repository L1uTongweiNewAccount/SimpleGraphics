...
#define DRIVER_NAME	"SimpleGraphics"
#define DRIVER_DESC	"DRM driver for SimpleGraphics platform devices"
#define DRIVER_DATE	__DATE__
#define DRIVER_MAJOR 1
#define DRIVER_MINOR 0

static int SimpleGraphics_get_validated_int(struct drm_device *dev, const char *name, uint32_t value){
	if (value > INT_MAX) {
		drm_err(dev, DRIVER_NAME ": invalid framebuffer %s of %u\n", name, value);
		return -EINVAL;
	}
	return (int)value;
}

static int SimpleGraphics_get_validated_int0(struct drm_device *dev, const char *name, uint32_t value){
	if (!value) {
		drm_err(dev, DRIVER_NAME ": invalid framebuffer %s of %u\n",
			name, value);
		return -EINVAL;
...
static const struct drm_format_info *SimpleGraphics_get_validated_format(struct drm_device *dev, const char *format_name)
{
